...
def compute_reaction_force_y(u, ax, ay, bx, by, lx, ly, k1, k3, bottom_y_dofs):
    """
    Compute total vertical support reaction on the bottom boundary.

    Reaction at constrained DOFs is -f_int(DOF), where f_int is the
    assembled internal nodal force from all springs at the final state.
    """
    d_ext = lx * (u[bx] - u[ax]) + ly * (u[by] - u[ay])
    F = k1 * d_ext + k3 * d_ext**3
